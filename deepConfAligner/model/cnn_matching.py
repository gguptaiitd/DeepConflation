...
from cnn_layers import param_init_encoder, encoder, param_load_encoder
...
def load_params(options,b):
    ...
    length = len(options['filter_shapes'])
    for idx in range(length):
        params = param_load_encoder(options['filter_shapes'][idx], params,b, prefix=_p('cnn_encoder', idx))
